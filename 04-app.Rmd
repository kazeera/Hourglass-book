...
Before using, ensure R is installed (and rtools for Windows), see [Install R]. If you are unsure, open the R terminal and type the following command to see where R is located on your computer: 
```
.libPaths()
```
...
knitr::include_graphics(c("images/app/GUI_7_run-hourglass.png"))
```
...
* Finally! Press the "Run Hourglass" button which interfaces to R (back-end). Hourglass will read from the [Output: Excel File], mentioned at the end of this chapter.  

This action will relay user options and data file paths to the Hourglass R package and calls on functions to run comparisons using established R plotting libraries, such as ggplot2 and pheatmap. An error window will pop up if there are any issues running Hourglass. 
...
```{r, echo=FALSE, out.width="80%", fig.align = "center", fig.show='hold', "Example of Comparisons worksheet in output Excel file."}
...
If you'd like to **exclude an option**(e.g. "Sex-X" due to low patient number), set to white "#ffffffff" or do not include and it will be removed from all comparisons.
...
```{r, echo=FALSE, out.width="47%", fig.align = "center", fig.show='hold', fig.cap="Example of Colors worksheet in output Excel file."}
...
```{r, echo=FALSE, out.width="66%", fig.align = "center", fig.show='hold', fig.cap="Example of Feature Sets worksheet in output Excel file."}
...
```{r, echo=FALSE, out.width="66%", fig.align = "center", fig.show='hold', fig.cap="Example of Feature Parameters worksheet in output Excel file."}
